...
APP_NAME=wya-test-1.0.0-SNAPSHOT.jar
APP_USER=tomcat
...
start(){
  ...
  if [ $? -eq "0" ]; then
    ...
    nohup java $JAVA_OPTS -jar $workdir/$APP_NAME --server.port=8081 > /dev/null 2>&1 >nohup.8081 &
    #nohup java $JAVA_OPTS -jar $workdir/$APP_NAME --server.port=8092 > /dev/null 2>&1 >nohup.8082 &
    is_exist
    if [ $? -eq "0" ]; then
